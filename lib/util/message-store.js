...
var MessageStore = exports.MessageStore = function(messages)
{
	...
	{
        var message = {
	    	meta: ((typeof messages[i].meta === "object")?messages[i].meta:JSON.parse(messages[i].meta || "{}")),
	    	og: messages[i].data
	    };
        message.originalMeta = {};
        Object.keys(message.meta).forEach(function (name) {
            message.originalMeta[name] = message.meta[name];
        });
...
MessageStore.prototype.getMessagesForTarget = function(targetId)
{
    ...
    try {
        ...
        Object.keys(groupStack).forEach(function(name) {
            if(groupStack[name].length>0) {
                // TODO: Display warning to user
                system.log.warn("Group nesting not correct: Not all opened groups were closed again! Parent group: " + name);
            }
        });
